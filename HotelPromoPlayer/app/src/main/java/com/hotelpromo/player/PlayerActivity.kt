...
class PlayerActivity : AppCompatActivity() {
    ...
    /**
     * Runs the same pointer-file check as the settings screen, but with
     * no UI at all — playback (from whatever's already on disk) is
     * already underway by the time this is called. If it turns out
     * there's a new video, it downloads in the background and this
     * calls playVideo() again once it's ready, swapping it in. If
     * there's nothing new, or the check fails for any reason (no WiFi,
     * bad URL, low storage, etc.), playback just continues undisturbed
     * — this is purely opportunistic.
     */
    private fun checkForUpdateSilently() {
        if (DownloadConfig.VIDEO_DOWNLOAD_URL.isBlank()) return

        VideoUpdateManager.checkForUpdate(
            context = this,
            onProgress = { /* no UI to update — silent by design */ },
            onResult = { result ->
                if (!isFinishing && !isDestroyed && result is VideoUpdateManager.Result.Updated) {
                    // Prefs already points at the new file — reuse the
                    // normal playback path to swap it in.
                    playVideo()
                }
                // NoUpdateNeeded or Failed: nothing to do, keep playing
    ...
    // Back button is disabled — this is a kiosk-style app, guests
    // shouldn't be able to exit to the TV's home screen from here.
    // The only way to reach the settings screen is typing 2580.
